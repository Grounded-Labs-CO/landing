...
function SignInForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // El redirect ocurre cuando isAuthenticated se actualiza.
    } catch {
      setError(
        mode === "signIn"
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-16">
      ...
      <div className="mt-8 flex border border-[#262E31]">
        <button
          ...
          className={`flex-1 px-4 py-3 font-mono text-[11px] tracking-[0.12em] uppercase transition-colors ${
            mode === "signIn"
              ? "bg-[#1C2427] text-[#F1F3F2]"
              : "text-[#6C7573] hover:text-[#9AA3A1]"
          }`}
        >
          ingresar
        </button>
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
        ...
        {error && (
          <p className="border border-[#3A1C0C] bg-[#1C2427] px-4 py-3 font-mono text-[12px] leading-[1.6] text-[#E2A084]">
            {error}
          </p>
        )}
